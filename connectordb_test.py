...
class TestConnectorDB(unittest.TestCase):
	def setUp(self):
		...
		self.usrdb = connectordb.ConnectorDB("python_test","mypass",url=TEST_URL)
	def tearDown(self):
		self.usrdb.close()
		try:
			self.usr.delete()
	...
	def test_authfail(self):
		try:
			db = connectordb.ConnectorDB("notauser","badpass",url=TEST_URL)
	...
	def test_counting(self):
		...
		self.assertRaises(connectordb.AuthenticationError,self.usrdb.count_streams)
		self.assertRaises(connectordb.AuthenticationError,self.usrdb.count_devices)
		self.assertRaises(connectordb.AuthenticationError,self.usrdb.count_streams)
	...
	def test_user(self):
		...
		self.assertRaises(connectordb.AuthenticationError,self.usr.set,{"admin": "Hello"})
	...
	def test_stream(self):
		self.assertRaises(SchemaError,self.usrdb["mystream"].create,{"type": "blah blah"})
	...
	def test_struct(self):
		...
		s.create({"type":"object","properties":{
			"test": {"type": "string"},
			"t2": {"type": "number"}
		}})
